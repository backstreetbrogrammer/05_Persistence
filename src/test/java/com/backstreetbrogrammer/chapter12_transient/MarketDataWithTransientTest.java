...
public class MarketDataWithTransientTest {

    private File serFile;
    private final MarketDataProviderTransient marketDataProvider = new MarketDataProviderTransient("BBG");

    @BeforeEach
    void setUp() throws IOException {
        final var serPath = Path.of("src", "test", "resources", "MarketDataWithTransientTest.ser");
    ...
    @Test
    @DisplayName("Test basic serialization for Java POJO with transient keyword")
    void testSerialize() throws IOException {
        final var marketData = new MarketDataWithTransient();
    ...
    @Test
    @DisplayName("Test basic deserialization for Java POJO with transient keyword")
    void testDeserialize() throws IOException, ClassNotFoundException {
        try (final var ois = new ObjectInputStream(
            ...
            final var fromSerialize = (MarketDataWithTransient) ois.readObject();
            ...
            assertEquals(0.0D, fromSerialize.getLast());
            assertFalse(fromSerialize.isLevelOne());
            assertNull(fromSerialize.getMarketDataProvider());
        }
    }
