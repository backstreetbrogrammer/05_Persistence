...
public class MarketDataWithEnumTest {

    private File serFile;

    @BeforeEach
    void setUp() throws IOException {
        final var serPath = Path.of("src", "test", "resources", "MarketDataWithEnumTest.ser");
    ...
    @Test
    @DisplayName("Test basic serialization for Java POJO with enum member fields")
    void testSerialize() throws IOException {
        final var marketData = new MarketDataWithEnum();
        ...
        marketData.setTradeStatus(TradeStatus.LIVE);
    ...
    void testDeserialize() throws IOException, ClassNotFoundException {
        try (final var ois = new ObjectInputStream(
            ...
            final var fromSerialize = (MarketDataWithEnum) ois.readObject();
            ...
            assertEquals(TradeStatus.LIVE, fromSerialize.getTradeStatus());
        }
    }
