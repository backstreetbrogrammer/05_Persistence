...
public class MarketDataWithInheritanceTest {

    private File serFile;
    private final MarketDataProviderInheritance marketDataProvider = new MarketDataProviderInheritance("REUTERS");

    @BeforeEach
    void setUp() throws IOException {
        final var serPath = Path.of("src", "test", "resources", "MarketDataWithInheritanceTest.ser");
    ...
    @Test
    @DisplayName("Test basic serialization for Java POJO with inheritance")
    void testSerialize() throws IOException {
        final var marketData = new MarketDataWithInheritance();
    ...
    @Test
    @DisplayName("Test basic deserialization for Java POJO with inheritance")
    void testDeserialize() throws IOException, ClassNotFoundException {
        try (final var ois = new ObjectInputStream(
            ...
            final var fromSerialize = (MarketDataWithInheritance) ois.readObject();
            ...
            assertEquals(marketDataProvider, fromSerialize.getMarketDataProvider());

            assertNull(fromSerialize.getId());
        }
    }
}
